    ...
    <div class="modal-dialog">
        <form action="{{ route('sizes.update', $size->id) }}" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <!-- Status -->
                <div class="mb-3">
                    <label for="status{{ $size->id }}" class="form-label">Status</label>
                    <select name="status" class="form-select" id="status{{ $size->id }}">
                        <option value="1" @selected($size->status == 1)>Active</option>
                        <option value="0" @selected($size->status == 0)>Inactive</option>
                    </select>
                </div>
            </div>
